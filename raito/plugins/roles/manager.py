...
class RoleManager:
    ...
    async def get_role(self, user_id: int) -> Role | None:
        ...
        """
        return (
            Role.DEVELOPER if self.is_developer(user_id) else await self.provider.get_role(user_id)
        )
    ...
    async def _check_role_hierarchy(self, initiator_id: int, target_id: int) -> None:
        initiator_role = await self.get_role(initiator_id)
    ...
    async def assign_role(self, initiator_id: int, target_id: int, role: Role) -> None:
        ...
        if not await self.can_manage_roles(initiator_id):
        ...
        await self._check_role_hierarchy(initiator_id, target_id)
        await self.provider.set_role(target_id, role)

    async def revoke_role(self, initiator_id: int, target_id: int) -> None:
        ...
        await self._check_role_hierarchy(initiator_id, target_id)
        await self.provider.remove_role(target_id)
